...
export default function Comment({ data, hasNext }) {
    ...
    function handleCommentReplySubmit(ev) {
        ...
        celesupBackendApi
            ...
            .then((res) => {
                ...
                commentsContext.insertToCommentReplies(comment, reply)
            })
            .catch((err) => {})
    }

    function renderParentName(cmm, author = false) {
        function render(reply, idx) {
            if (reply.parent) {
                return (
                    <>
        ...
        if (cmm.parent?.id) {
            return render(comment.parent, 0, false)
        }
