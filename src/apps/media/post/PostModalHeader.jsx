...
export default function PostModalHeader({ dispatcher, options = {} }) {
    ...
    useEffect(() => {
        ...
        if (Object.keys(form).length > 0) {
            setConfiguration(pages, setConfig, config, form, storeDispatch)
        }
    }, [form])
    ...
    useEffect(() => {
        ...
        setConfiguration(pages, setConfig, _config, form, storeDispatch)
    }, [pages, validForm])
...
const HEADER_BUTTONS = {
    closeBtn: <span className="text-xl font-semibold">&times;</span>,
    backBtn: (
...
function setConfiguration(pages, setConfig, config, form, storeDispatch) {
...
async function submitPostForm(form, storeDispatch) {
    const data = { ...form }
    if (data.picture) {
        const { file } = await createFileFromDataUrl(data.picture)
        data["picture"] = file
    }
